        ...
        for seed_file in valid_seeds:
            ...
            if not success:
                if "timeout" in output.lower():
                    ...
                    log_error(f"种子执行超时 - Harness: {harness_name}, 种子: {seed_file.name}, 错误: {output[:5000]}")
                elif "crash" in output.lower() or return_code < 0:
                    test_result['crashed'] = True
                    log_error(f"种子执行崩溃 - Harness: {harness_name}, 种子: {seed_file.name}, 返回码: {return_code}, 错误: {output[:5000]}")
                else:
                    log_error(f"种子执行失败 - Harness: {harness_name}, 种子: {seed_file.name}, 返回码: {return_code}, 错误: {output[:5000]}")
